...
var validateStage = middleware.validateStage;
...
router.post('/users/:username/projects/:project/commit', validateCommit, function (req, res, next) {
  ...
  Project.findOne({
    ...
    project.commitFiles(message, status, (err, statuses) => {
      if (err) return next(err);
      res.json(statuses);
    });
  });
...
router.post('/users/:username/projects/:project/stage', validateStage, function (req, res, next) {
  ...
  Project.findOne({
    ...
    project.stageFiles(files, status, (err, statuses) => {
      if (err) return next(err);
      res.json(statuses);
    });
  });
...
router.get('/users/:username/projects/:project/path/*', setReqBody, normalizePath, validatePath, function (req, res, next) {
  Project.findOne({
    creator: req.params.username,
    project_name: req.params.project
  }, (err, project) => {
