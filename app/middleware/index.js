...
module.exports.validateFile = function (req, res, next) {
  let fileName = req.params.path.slice(req.params.path.lastIndexOf('/') + 1);
  if (req.body.type === 'F') {
    match = /^[a-zA-Z0-9]+.(html|js|css)$/g.exec(fileName);
    if (!match || !match[1]) {
      return next({
        status: 400,
        message: "You can only create html, css or javascript files"
      });
    }
    ...
    if (!match) {
      return next({
        status: 400,
        message: "Directory names must be alphanumeric"
      });
    }
  } else {
    return next({
...
module.exports.validatePath = function (req, res, next) {
  ...
  return next({
    status: 400,
    message: 'Invalid path. Directories must be alphanumberic and files must be alphanumeric and end in .html, .css or .js'
  });
};

module.exports.validateCommit = function (req, res, next) {
  if (!req.body.message || typeof req.body.message !== 'string' || req.body.message.length === 0) {
    return next({
      status: 400,
...
module.exports.validateStage = function (req, res, next) {
  if (!req.body.files || req.body.files.constructor !== Array || req.body.files.length == 0) {
    ...
    if (error) {
      return next({
        status: 400,
        message: 'Files are malformed. Must contain a path and type'
      });
    }
    next();
  }
