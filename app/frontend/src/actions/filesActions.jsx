...
export const updateFileContents = updatedContents => ({
  type: "UPDATE_FILE_CONTENTS",
...
export const openFile = path => (dispatch, getState) => {
  ...
  let index = state.files.open.findIndex(file => file.path === path);
  if (index === -1) {
  ...
  return axios
    .get(
      `/api/projects/users/${creator}/projects/${project_name}/path/${path.slice(
        1
      )}`
    )
...
export const viewProjectNewTab = () => (dispatch, getState) => {
  ...
  var { creator, project_name } = state.project.metadata;
  window.open(
    `/staticcontent/users/${creator}/projects/${project_name}/`,
    "_blank"
  );
};
